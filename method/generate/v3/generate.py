"""Streaming, multi-GPU Qwen3 generation with per-worker continuous batches."""
...
DEFAULT_CONFIG = Path(__file__).resolve().parents[2] / 'calibrate_leakage_detector/output/detector_config.json'
...
def parser(output_required=True):
    ...
    result.add_argument('--model-key', default='Qwen3-4B')
...
def prepare_output(args):
    ...
    metadata = {'version': 3, 'input_sha256': file_hash(args.input),
...
def load_runtime(args, device):
    ...
    if transformers.__version__ != '4.57.6':
        raise RuntimeError('v3 cache/attention integration requires transformers==4.57.6 (see requirements.txt).')
    torch.set_num_threads(args.cpu_threads)
    if device.startswith('cuda'):
        torch.cuda.set_device(device)
    with open(args.detector_config, encoding='utf-8') as source:
        detector_config = json.load(source)
    ...
    if model.config.model_type != 'qwen3' or getattr(model.config, 'use_sliding_window', False):
        raise ValueError('v3 currently supports full-attention Qwen3 models only.')
    rope = getattr(model.config, 'rope_scaling', None)
