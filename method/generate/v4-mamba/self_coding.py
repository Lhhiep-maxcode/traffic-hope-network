...
try:
    from .detector import resolve_heads
except ImportError:
    from detector import resolve_heads
...
class TokenByTokenGenerator:
    """Token generation with recurrent decode and exact-prefix replay on rollback.

    ``cache_mode='recompute'`` is a slow full-prefill reference for validation.
    Cached decode may differ numerically from full prefill on CUDA.
    """
    ...
    def __init__(
        ...
        self.model = model
        if cache_mode not in {"cached", "recompute"}:
            raise ValueError("cache_mode must be cached or recompute.")
        ...
        ) if getattr(tokenizer, "chat_template", True) else raw_full_prompt
    ...
    def _is_eos(self, token_id):
        ids = getattr(getattr(self.model, "generation_config", None), "eos_token_id", None)
        ids = ids if isinstance(ids, (list, tuple)) else [ids]
        return token_id in [i for i in [*ids, self.tokenizer.eos_token_id] if i is not None]

    def _remember_logits(self):
        # v4 never uses the v3 KV-cropping/logit-history rollback shortcut.
        pass

    @torch.no_grad()
    def _restore_prefix(self, generated_ids):
        """Rebuild shortened/dirty SSM state; replay append-only suffixes one token at a time."""
        generated_ids = [int(token_id) for token_id in generated_ids]
        current = list(self.generated_ids)
        if (not self.reuse_kv_cache or self._cache_dirty or self.past_key_values is None
                or self._tokenized_prompt != self.full_prompt
                or generated_ids[:len(current)] != current):
            return self.start(generated_ids)
        for token_id in generated_ids[len(current):]:
            outputs = self._forward_token(token_id)
            self._accept(token_id, outputs)
        return self._prompt_ids.numel()

    @torch.no_grad()
    def start(self, generated_ids=None):
        """Recompute both attention and recurrent states from the exact prefix."""
        ...
        input_ids = torch.cat([prompt_ids, torch.tensor(generated_ids, dtype=torch.long)])
        input_ids = input_ids.unsqueeze(0).to(_model_device(self.model))
        self.past_key_values = None
        self.next_logits = None
        self._logits_history = {}
        outputs = self.model(input_ids=input_ids, use_cache=self.cache_mode == "cached",
                             output_attentions=False, cache_length=0)
        self.past_key_values = outputs.past_key_values
        if self.cache_mode == "cached" and self.past_key_values is None:
            raise RuntimeError("The model did not return recurrent state. Use --cache-mode recompute to diagnose.")
        self.next_logits = outputs.logits[:, -1, :].clone()
        self.cache_length = input_ids.shape[1]
        self.generated_ids = generated_ids
        self._cache_dirty = False
        self.finished = bool(generated_ids and self._is_eos(generated_ids[-1]))
    ...
    def _forward_token(self, token_id, output_attentions=False):
        if self._cache_dirty:
            raise RuntimeError("A speculative token must be accepted or rolled back before another forward.")
        if self.cache_mode == "recompute":
            ids = [*self._prompt_token_ids().tolist(), *self.generated_ids, int(token_id)]
            cache, length = None, 0
        else:
            ids = [int(token_id)]
            cache, length = self.past_key_values, self.cache_length
        # Native recurrent kernels mutate cache in place. A rejected candidate
        # invalidates it even if generated_ids/cache_length have not advanced.
        self._cache_dirty = True
        return self.model(
            input_ids=torch.tensor([ids], dtype=torch.long, device=_model_device(self.model)),
            past_key_values=cache, cache_length=length,
            use_cache=self.cache_mode == "cached", output_attentions=output_attentions,
        )
    ...
    def _accept(self, token_id, outputs):
        ...
        self.finished = self._is_eos(token_id)
...
class CustomGenerator(TokenByTokenGenerator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.heads = resolve_heads(self.detector, model.attention_layers)
    ...
    def _attention_score(self, attentions):
        if not attentions:
            raise RuntimeError("The model did not return attention weights for the detector.")
        ...
        for position, record in enumerate(self.heads):
            ...
            if layer not in attentions:
                raise ValueError(f"Detector layer {layer} is not an attention block in this model.")
            attention = attentions[layer][0, int(record["head"]), -1]
            ...
            if self.fix_comparison_method == "js_divergence":
                ...
                if candidate_needs_repair or self.debug_wait_safe_window:
                    ...
                    if (
                        ...
                        # Rebuild recurrent state after rejecting the candidate.
                        privileged_outputs = None
        ...
        if not repair_continues or self.finished:
            ...
            details = {
                ...
                "comparison_threshold": (
                    self.threshold if self.fix_comparison_method == "attention_score"
                    else self.fix_js_divergence_threshold
                ),
                "repair_steps": self.repair_steps,
                "max_repair_steps": self.max_repair_steps,
                "safe_steps": self.repair_safe_steps,
                "required_safe_steps": self.repair_required_safe_steps,
                "best_safe_steps": max(step["safe_steps"] for step in event["steps"]),
                "window_size": self.window_size,
                "detected_index": event["detected_index"],
                "start_index": event["start_index"],
                "clean_backtrack_steps": len(event["clean_backtrack_token_ids"]),
                "recent_checks": [
                    {key: step[key] for key in (
                        "token_index", "comparison_score", "passed_check", "safe_steps",
                    )}
                    for step in event["steps"][-8:]
                ],
            }
            raise RuntimeError(
    ...
    def consume_candidate(self, candidate_id, outputs):
        """Apply the calibrated attention detector, then v3's repair rules."""
