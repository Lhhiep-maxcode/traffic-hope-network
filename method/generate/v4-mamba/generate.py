"""Streaming multi-GPU Nemotron/Mamba generation with recurrent-state-safe repair."""
...
DEFAULT_CONFIG = Path(__file__).with_name('detector_config.json')
...
def parser(output_required=True):
    ...
    result.add_argument('--model-key', required=True, help='Calibrated entry, e.g. Nemo3-Nano-4B-BF16-math.')
    result.add_argument('--detector-config', type=Path, default=DEFAULT_CONFIG)
    # Accept existing attention launchers; there is no alternative detector.
    result.add_argument('--detector-mode', choices=['attention'], default='attention', help=argparse.SUPPRESS)
    result.add_argument('--trust-remote-code', action=argparse.BooleanOptionalAction, default=True)
    result.add_argument('--cache-mode', choices=['cached', 'recompute'], default='cached')
    result.add_argument('--ssm-state-dtype', choices=['float32', 'model'], default='float32', help='Nemotron-H recurrent SSM state precision.')
    result.add_argument('--devices', default='auto', help='auto, cpu, or CUDA indices such as 0,1,2,3')
    result.add_argument('--batch-size', type=int, default=4, help='Active slots per GPU; only equal-length prefixes share a decode forward.')
    result.add_argument('--attention-backend', choices=['sdpa', 'eager'], default='sdpa')
    ...
    result.add_argument('--enable-thinking', action=argparse.BooleanOptionalAction, default=True)
...
def prepare_output(args):
    ...
    metadata = {'version': 4, 'runtime': 'v4-mamba', 'input_sha256': file_hash(args.input),
                'detector_sha256': file_hash(args.detector_config),
                'detector_layer_indexing': 'attention_order',
...
def preflight(args):
    """Validate once before creating outputs or launching eight model copies."""
    import math
    if (args.batch_size < 1 or args.cpu_threads < 1 or args.max_new_tokens < 0
            or (args.limit is not None and args.limit < 0)):
        raise ValueError('Invalid batch size, CPU thread count, or token/sample limit.')
    if not math.isfinite(args.temperature) or (args.do_sample and args.temperature <= 0):
        raise ValueError('Sampling temperature must be finite and positive.')
    if not 0 < args.top_p <= 1 or args.top_k < 0:
        raise ValueError('top_p must be in (0, 1], top_k must be nonnegative.')
    if args.max_repair_steps < 1 or not math.isfinite(args.fix_js_divergence_threshold) or args.fix_js_divergence_threshold < 0:
        raise ValueError('Invalid repair step limit or JSD threshold.')
    key, payload = load_detector(args)
    backtrack = (payload[key]['window_size'] - 1) // 2
    minimum = 1
    if args.debug_wait_safe_window:
        minimum = max(1, backtrack) + (backtrack if args.debug_clean_backtrack else 0)
    if args.debug_fix_infinite_loop or args.debug_clean_backtrack:
        minimum = max(minimum, backtrack + 1)
    if args.max_repair_steps < minimum:
        raise ValueError(f'max-repair-steps must be at least {minimum} for this repair window.')
    configure_libraries()
    import transformers
    from transformers import AutoConfig
    if transformers.__version__ != '4.57.6':
        raise RuntimeError('v4 integration requires transformers==4.57.6 (see requirements.txt).')
    config = AutoConfig.from_pretrained(args.model, revision=args.revision,
                                       trust_remote_code=args.trust_remote_code)
    if config.model_type not in ('nemotron_h', 'mamba', 'mamba2', 'falcon_mamba'):
        raise ValueError(f'Unsupported model_type={config.model_type!r}; use v3 for Qwen3.')
    pattern = getattr(config, 'hybrid_override_pattern', '')
    attention_layers = {i: config.num_attention_heads for i, value in enumerate(pattern) if value == '*'}
    resolve_heads(payload[key], attention_layers)
    if config.model_type == 'nemotron_h':
        for module, package in (('causal_conv1d', 'causal-conv1d'), ('mamba_ssm', 'mamba-ssm')):
            try:
                importlib.import_module(module)
            except (ImportError, OSError) as error:
                raise RuntimeError(
                    f'Cannot import {package} using {sys.executable}: {error}\n'
                    'Run check_environment.py --cuda with this interpreter; '
                    'dependency versions are listed in requirements-cuda.txt.'
                ) from error
    return config
...
def load_runtime(args, device):
    ...
    key, detector_config = load_detector(args)
    tokenizer = AutoTokenizer.from_pretrained(args.model, revision=args.revision,
                                              trust_remote_code=args.trust_remote_code)
    model = AutoModelForCausalLM.from_pretrained(
        args.model, revision=args.revision, trust_remote_code=args.trust_remote_code,
        dtype=getattr(torch, args.dtype), device_map={'': device},
    ).eval()
    model = make_runtime(model, args.attention_backend,
                         torch.float32 if args.ssm_state_dtype == 'float32' else getattr(torch, args.dtype))

    def make_generator(sample):
        return CustomGenerator(
            model=model, tokenizer=tokenizer, detector_config_path=None,
            detector_config=detector_config, model_key=key,
            ...
            enable_thinking=args.enable_thinking, decode_tokens=args.decode_tokens,
...
def main():
    ...
    if config.model_type == 'nemotron_h' and 'cpu' in devices:
        raise ValueError('NVIDIA remote Nemotron-H mixers require CUDA; native Mamba CPU is supported for tests.')
...
def execute(args, devices):
    ...
    print(f'{len(devices)} workers | {args.batch_size} slots/worker | equal-length batching | '
          f'{args.detector_mode} detector | {len(completed)} samples already saved', file=sys.stderr)
