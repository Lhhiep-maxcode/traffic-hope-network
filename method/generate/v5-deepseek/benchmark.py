...
try:
    ...
    from .generate import file_hash, iter_samples, load_runtime, parser, resolve_devices
except ImportError:
    from batching import BatchScheduler
    from generate import file_hash, iter_samples, load_runtime, parser, resolve_devices
...
def main():
    ...
    options.add_argument('--report', type=Path, default=Path('benchmark-v5-deepseek.json'))
    ...
    report = {
        'model': args.model, 'model_key': args.model_key, 'revision': args.revision,
        'detector_sha256': file_hash(args.detector_config), 'device': device,
