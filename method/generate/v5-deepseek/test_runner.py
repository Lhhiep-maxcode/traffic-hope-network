...
class RunnerTests(unittest.TestCase):
    ...
    def test_resume_rejects_changed_input_and_recovers_partial_tail(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = parser().parse_args(['--input', str(source), '--output', str(output),
                                       '--detector-config', str(config), '--model-key', 'test'])
    ...
    def test_two_spawned_workers_with_real_local_tiny_model(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            backend = Tokenizer(WordLevel({'[UNK]': 0, 'Q': 1, '0': 2, '[EOS]': 3}, unk_token='[UNK]'))
            ...
            self.assertTrue(all(1 <= len(row['token_ids']) <= 4 for row in rows))
            self.assertTrue(all(len(row['token_ids']) == 4 or row['token_ids'][-1] == 3 for row in rows))
