"""Small real Qwen2 tensor/cache regressions; no model download required."""
...
class BatchTests(unittest.TestCase):
    ...
    def test_selective_multi_layer_prefill_and_cached_suffix_are_causal(self):
        model = copy.deepcopy(self.model)
        detector = {**self.detector, 'heads': [{'layer': 0, 'head': 1}, {'layer': 2, 'head': 3}]}
        enable_selective_attention(model, detector)
        for prefix in ([], [7, 8, 9]):
            with self.subTest(prefix=prefix), torch.no_grad():
                eager_cache = selective_cache = None
                if prefix:
                    ids = torch.tensor([prefix])
                    eager_cache = self.model(ids, use_cache=True).past_key_values
                    selective_cache = model(ids, use_cache=True).past_key_values
                ids = torch.tensor([[2, 3, 4, 5]])
                expected = self.model(ids, past_key_values=eager_cache, use_cache=True, output_attentions=True)
                actual = model(ids, past_key_values=selective_cache, use_cache=True,
                               attention_mask={'full_attention': None}, output_attentions=True)
                torch.testing.assert_close(actual.logits, expected.logits, atol=1e-6, rtol=1e-5)
                self.assertEqual(len(actual.attentions), 3)
                self.assertEqual(actual.attentions[1].numel(), 0)
                for layer in (0, 2):
                    torch.testing.assert_close(actual.attentions[layer], expected.attentions[layer],
                                               atol=1e-6, rtol=1e-5)
                    self.assertTrue(torch.all(actual.attentions[layer][..., 0, len(prefix) + 1:] == 0))

    def test_batched_rollback_and_replacement_match_fresh_prefill(self):
        for selective in (False, True):
            model = copy.deepcopy(self.model)
            if selective:
                enable_selective_attention(model, self.detector)
            for prefix in ([2, 3, 4], [2, 9, 10], []):
                with self.subTest(selective=selective, prefix=prefix):
                    samples = [{'sample_index': i, 'question': 'Q' * (i * 5 + 1)} for i in range(3)]
                    generators = [self.generator(s, model=model) for s in samples]
                    for g in generators:
                        g.begin()
                    for token in (2, 3, 4, 5):
                        outputs = forward_batch(generators, [token] * len(generators))
                        for g, output in zip(generators, outputs):
                            g._accept(token, output)
                    for sample, g in zip(samples, generators):
                        g._restore_prefix(prefix)
                        fresh = self.generator(sample, model=model)
                        fresh.start(prefix)
                        torch.testing.assert_close(g.next_logits, fresh.next_logits, atol=1e-6, rtol=1e-5)
                        for a, b in zip(g.past_key_values.layers, fresh.past_key_values.layers):
                            torch.testing.assert_close(a.keys, b.keys, atol=1e-6, rtol=1e-5)
                            torch.testing.assert_close(a.values, b.values, atol=1e-6, rtol=1e-5)

    def test_selective_repair_matches_eager_for_both_comparison_methods(self):
        model = copy.deepcopy(self.model)
        enable_selective_attention(model, self.detector)
        samples = [{'sample_index': i, 'question': 'Q' * (i + 1)} for i in range(4)]
        for method in ('attention_score', 'js_divergence'):
            kwargs = {'repair': True, 'do_sample': False, 'fix_comparison_method': method,
                      'fix_js_divergence_threshold': 2.0}
            expected = {s['sample_index']: self.generator(s, **kwargs).generate(True, False) for s in samples}
            for size in (1, 2, 4):
                with self.subTest(method=method, batch_size=size):
                    rows = list(BatchScheduler(lambda s: self.generator(s, model=model, **kwargs),
                                               size, include_unfixed=True).run(samples))
                    for row in rows:
                        reference = expected[row['sample_index']]
                        self.assertTrue(row['repair_events'])
                        for key in ('token_ids', 'fixed', 'unfixed'):
