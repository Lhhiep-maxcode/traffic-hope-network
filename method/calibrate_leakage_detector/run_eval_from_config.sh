...
CONFIG_PATH="${CONFIG_PATH:-${SCRIPT_DIR}/output/detector_config_new.json}"
...
split_key() {
  ...
  for domain in "${domains[@]}"; do
    local suffix="-${domain}"
    if [[ "${key}" == *"${suffix}" ]]; then
      MODEL_NAME="${key%"${suffix}"}"
      DOMAIN="${domain}"
      return 0
    fi
  done

  echo "Cannot infer domain from config key: ${key}" >&2
  return 1
}
...
for MODEL_KEY in "${CONFIG_KEYS[@]}"; do
  ...
  PLOT_DIR="method/calibrate_leakage_detector/output/${MODEL_NAME}/${MODEL_KEY}/plots"
